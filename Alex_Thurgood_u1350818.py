...
def write_memory():
    for routers in ["pa3-r1-1", "pa3-r2-1", "pa3-r3-1", "pa3-r4-1"]:
        subprocess.run(f"docker exec {routers} vtysh -c 'write memory'", shell=True, check=True)

def ospf_north():
    # Set OSPF route to use R1 → R2 → R3
    set_ospf_cost("pa3-r1-1", "10.0.10.4", 2)   
    set_ospf_cost("pa3-r1-1", "10.0.13.4", 50)  
    set_ospf_cost("pa3-r2-1", "10.0.11.3", 2)   
    set_ospf_cost("pa3-r4-1", "10.0.12.4", 50) 
    set_ospf_cost("pa3-r3-1", "10.0.11.4", 2)  
    set_ospf_cost("pa3-r3-1", "10.0.12.3", 50) 
    write_memory()
    subprocess.run("docker exec pa3-r3-1 vtysh -c 'clear ip ospf route'", shell=True, check=True)
...
def ospf_south():
    ...
    write_memory()
    subprocess.run("docker exec pa3-r3-1 vtysh -c 'clear ip ospf process'", shell=True, check=True)
